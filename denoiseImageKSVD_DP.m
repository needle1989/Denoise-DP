function [IOut,output] = denoiseImageKSVD_DP(Image,sigma,K,blockSize)
%==========================================================================
...
if(prod([NN1,NN2]-blockSize+1)> maxNumBlocksToTrainOn)
    ...
    blkMatrix = im2col(Image,[blockSize,blockSize],'sliding');%%%%%%%8*8=64   所以blkMatrix矩阵大小为：64*[（NN1-bb+1）*(NN2-bb+1)]
end
...
%     DCT(:,k+1)=V/norm(V);
% end
% DCT=kron(DCT,DCT);%%%%%跟DCT中的代码一样的     64*256的矩阵  ****param.numIteration
...
[Dictionary,output] = KSVD_DP(blkMatrix,param);%%%%%%%最核心的函数%%%%%%%%%%%%
output.D = Dictionary;
...
%     x=1:size(full_Coefs,1);
    [Coefs,~,~] = bmtl_DP(Dictionary,blocks(:,jj:jumpSize));
    %plot(x,full_Coefs);
    if (reduceDC)
