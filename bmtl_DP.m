...
%     [hyper.alpha,temp] = kmeans(hyper.alpha.',hyper.truncateL);
    ...
    for ii = 1:M
        ...
        hyper.alpha(:,ii) =  (1 ./ abs(Phi' * Y(:,ii)));%ones(N,1)*inf
%         hyper.alpha(:,ii) = ones(N,1)*inf;
        hyper.mu(:,ii)=Phi' * Y(:,ii);
    end
     hyper.kappa = init_kappa(M);
%      hyper.kappa = abs(ones(hyper.truncateL,M));
%      hyper.kappa = hyper.kappa ./ repmat(sum(hyper.kappa,1),hyper.truncateL,1);
%     hyper.kappa = eye(hyper.truncateL,M);
...
%     filename1 = strcat('result/exp5k/k',num2str(mmm),'.mat');
%     save(filename1, 'kappa');
%     filename2 = strcat('result/exp5x/x',num2str(mmm),'.mat');
%     X_reshape = Phi * mu; 
%     result = col22im(X_reshape, 128, 4);
...
%% Update mu
for ii = 1 : M;
    tmpC = inv(diag( sum (  alpha * diag( kappa(:,ii) ), 2)));
    C = 1 / alpha0 * eye(nd) + Phi * inv(diag( sum (  alpha * diag( kappa(:,ii) ), 2))) * Phi';
    Cinv = inv(C);
    Sigma(:,:,ii) = inv(diag( sum (  alpha * diag( kappa(:,ii) ), 2))) - inv( diag( sum (  alpha * diag( kappa(:,ii) ), 2)) ) * Phi' * Cinv * Phi * inv(diag( sum (  alpha * diag( kappa(:,ii) ), 2)));
    mu(:,ii) = alpha0 * Sigma(:,:,ii) * Phi' * Y( : , ii );
end  
...
bnew = b;
for ii = 1 : M
    bnew = bnew +abs(norm( Y(:,ii) - Phi * mu(:,ii), 'fro')^2)+ trace(Phi  * Sigma(:,:,ii) * Phi');
end
