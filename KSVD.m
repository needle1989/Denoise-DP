...
totalErr = zeros(1,param.numIteration);%param.numIteration = numIQV  CaW    SterOfKsvd=10 ;    %sigma=50   所以numIterOfKsvd = 10;  
    
% the K-SVD algorithm starts here.
%Kerr's changes
%blockSize=8;
blockSize=ceil(sqrt(size(Data,1)));
blkMatrix = im2col(Data,[blockSize,blockSize],'distinct');%一列表示一个块,有blockSize^2行
blkNum=size(blkMatrix,2);
blkMtx=zeros(size(blkMatrix));%用来存放已经处理好的块
for iterNum = 1:param.numIteration  %param.numIteration = numIterOfKsvd=10
	...
	rPerm = randperm(size(Dictionary,1));%size(Dictionary,2)=256  测试一下就知道该函数的用法了(产生1到256的随机的整数，没有重合的整数)
...
%errors =(Data(:,relevantDataIndices) - tmpCoefMatrix); % vector of errors that we want to minimize with the new element    D:64*256     tmpCoefMatrix尺寸为：256*length(relevantDataIndices)  Data(:,relevantDataIndices):64*relevantDataIndices
